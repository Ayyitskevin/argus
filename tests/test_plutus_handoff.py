...
def test_handoff_async_records_mise_callback(monkeypatch):
    ...
    def fake_callback(gallery_id, *, run_id=None, status="done", error=None):
        callbacks.append(
            {"gallery_id": gallery_id, "run_id": run_id, "status": status, "error": error}
        )
    ...
    time.sleep(0.2)
    assert callbacks == [{"gallery_id": 5, "run_id": 12, "status": "done", "error": None}]
